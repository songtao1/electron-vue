...
export const dbActions = class HandleDB {
    constructor(obj) {
        this.databaseFile = obj && obj.databaseFile || `./db/test.db`;// 数据库文件(文件路径+文件名)
        //this.tableName = obj && obj.tableName || `testTable`;   // 表名
    ...
    prepare(param){
        return this.db.prepare(param);
    }
    serialize(param){
        this.db.serialize(param);
    }
    exec(sql){
        const _self = this;
        // _self.db.exec(param);
        return new Promise((resolve, reject) => {
            _self.db.exec(sql,function (err, data) {
                    if (err) {
                        reject(new Error(err));
                    } else {
                        if (data) {
